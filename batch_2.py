    ...
    def train(self, Train_Image_Dir, Validate_Image_Dir, Epochs, Batch_Size, save_model_path, save_num = 10):
        ...
        with tf.Session() as sess:
            ...
            if ckpt and tf.train.checkpoint_exists(ckpt.model_checkpoint_path):
                print('exist global_variables load...')
                saver.restore(sess, ckpt.model_checkpoint_path)
                start_epoch = int(sess.run(self.global_step) / TrainReader.MaxIter)
            ...
            for epoch in range(start_epoch, Epochs):
                ...
                for iter in range(TrainReader.MaxIter):
                    ...
                    if epoch % save_num == 0 and epoch > 0:
                ...
                if epoch % save_num == 0 and epoch > 0:
                    if UseValidate:
                        ...
                        print("Step " + str(epoch) + " Validate Loss=" + str(VLoss / ValideReader.MaxIter))
                        print("Step " + str(epoch) + " Validate Acc=" + str(val_acc / ValideReader.MaxIter))

                        History_epoch.append(epoch)
                        History_TLoss.append(TLoss / TrainReader.MaxIter)
                        History_train_acc.append(train_acc / TrainReader.MaxIter)
                        History_VLoss.append(VLoss / ValideReader.MaxIter)
                        ...
                        if epoch % save_num == 0 and epoch > 0:
                            ...
                            # saver.save(sess, save_model_path + "model.ckpt", global_step=self.global_step)  # Save model
        ...
        loss_ax.set_xlabel('epoch_0.001', color='blue', fontsize=15)
